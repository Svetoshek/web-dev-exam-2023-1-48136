...
let guideSelect = document.querySelector('#guides-select');
let experienceFrom = document.querySelector('#experience-from');
let experienceTo = document.querySelector('#experience-to');
// модальное окно
...
async function buttonChooseGuideHandler(event) {
    ...
    excursionDate.value = getCurrentDate();
    option1Name.innerHTML = 'Легкие закуски и горячие напитки во время экскурсии';
    option1Desc.innerHTML = 'Увеличивает стоимость на 1000 рублей за каждого посетителя';
    option2Name.innerHTML = 'Интерактивный путеводитель';
    option2Desc.innerHTML = 'увеличивает стоимость в 1,5 раза ';
...
/**
 * Функция для подготовки "сырых" данных к отображению в селекторе
 * @param {object} data - "сырые" данные, содержащие данные о гидах
 * @returns {object} - "чистые" данные, готовые для отображения в селекторе
 */
function prepareSelectorGuides(data) {
    let setLanguage = new Set();
    for (let i = 0; i < Object.keys(data).length; i++) {
        let mainObject = data[i]['language'];
        if (mainObject.includes(' ')) {
            mainObject = mainObject.split(' ');
            for (let j = 0; j < mainObject.length; j++) {
                setLanguage.add(mainObject[j]);
            }
        } else {
            setLanguage.add(mainObject);
        }
    }
    let resultLanguage = [];
    // перевод set в array
    setLanguage.forEach((value) => {
        resultLanguage.push(value);
    });
...
/**
 * Функция, выводящая в селектор информацию о доступных языках (секция Гиды)
 * @param {object} data - массив данных для отображения
 */
function renderSelectorOfGuide(data) {
    guideSelect.innerHTML = '';
    let optionElem = document.createElement('option');
    optionElem.innerHTML = '';
    guideSelect.append(optionElem);
    // создание элементов и добавление их в селектор
    for (let i = 0; i < data.length; i++) {
        let optionElem = document.createElement('option');
        optionElem.innerHTML = data[i];
        guideSelect.append(optionElem);
    }
}
 ...
 * @param {string} language - выбранный язык("" или выбранный язык)
 * @param {string} from - значение опыта работы "от"
 * @param {string} to - значение опыта работы "до"
 */
function generateGuides(data, language, from, to) {
    // флаг, чтобы понять стандртная загрузка или с параметрами
    let flag = false;
    let newdata = [];
    // если есть параметр - язык
    if (language != "" && language != undefined) {
        ...
        data = [];
        for (let pos = 0; pos < newdata.length; pos++) {
            data.push(newdata[pos]);
        }
    }
    // если есть параметр - опыт работы (от)
    if (from != "" && from != undefined) {
        newdata = [];
        flag = true;
        for (let i = 0; i < data.length; i++) {
            if (data[i]['workExperience'] >= parseInt(from)) {
                newdata.push(data[i]);
            }
        }
        data = [];
    ...
    // если есть параметр - опыт работы (до)
    if (to != "" && to != undefined) {
        newdata = [];
        flag = true;
        for (let i = 0; i < data.length; i++) {
            if (data[i]['workExperience'] <= parseInt(to)) {
                newdata.push(data[i]);
            }
        }
        data = [];
    ...
    // если параметры обработаны, то загрузка "отфильтрованных" данных
    if (flag) {
        renderGuides(newdata);
        return;
    }
    renderGuides(data);
    let selectorData = prepareSelectorGuides(data);
    renderSelectorOfGuide(selectorData);

}

/**
 * Функция-обработчик для селектора гидов
 * @param {object} event - событие
 */
async function changeGuidesHandler(event) {
    let containerGuides = event.target.closest('.container');
    let language = guideSelect.value;
    let from = experienceFrom.value;
    let to = experienceTo.value;
    let nameOfRoute = containerGuides.querySelector('.guides-name-of-route');
    let idRoute = nameOfRoute.dataset.routeId;
    let data = await dataExchangeWithTheServer('get', 'routes', {}, idRoute);
    generateGuides(data, language, from, to);
}
...
async function buttonChooseRouteHandler(event) {
    ...
    document.querySelector('.guides-name-of-route').dataset.routeId = idRoute;
    generateGuides(data);
}
...
window.onload = function () {
    ...
    // назначение обработчика на изменение селектора доступного языка гида(-ов)
    guideSelect.onchange = changeGuidesHandler;
    // назначение обработчиков на изменение в полях выбора опыта гида
    experienceFrom.oninput = changeGuidesHandler;
    experienceTo.oninput = changeGuidesHandler;
