...
async function buttonChooseGuideHandler(event) {
  ...
  excursionDate.value = getCurrentDate();
  option1Name.innerHTML = "увеличения стоимости экскурсии";
  option1Desc.innerHTML = "Увеличивают стоимость на за каждого посетителя";
  option1amount.value = "1000 руб";
  option2Name.innerHTML = "трансфер после экскурсии";
  option2Desc.innerHTML =
    "трансфер до ближайших станций метро после \
...
async function buttonSendRequestHandler(event) {
  ...
  if (
    ...
    let text =
      "Заявка не может быть создана :(<br>\
...
function renderGuides(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (exp == 1) {
      itemGuides.querySelector(".exp").innerHTML = exp + " год";
    } else {
      if (exp < 5) {
        itemGuides.querySelector(".exp").innerHTML = exp + " года";
      }
      if (exp >= 5) {
        itemGuides.querySelector(".exp").innerHTML = exp + " лет";
      }
    }
